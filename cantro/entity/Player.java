...
public class Player {
	...
	public void shoot()
	{
		if(owner.gameScreen.status == Status.MULTIPLAYERGAME)
		owner.connection.send(""+Commands.SPAWNBULLET.val+":"+UID);
	...
	public String generateInputUpdateToSend()
	{
		...
		b+=40;
		char c = (char)b;
		if(input.down[Input.DOWN])input.remKey(Input.DOWN); //if down is pressed, remove it!
		return StringUtil.CombineWithColons(Commands.INPUTUPDATE.val,UID,c);
		/*
		return StringUtil.CombineWithColons(Commands.POSUPDATE.val,UID,x,y,xspeed,yspeed,isFacingRight);
		*/
	}
    ...
    public void goDown()
    {
    	...
    	{
    		if(pixels[i] == -16777216 || pixels[i] == -65281)	// PINK OR BLACK FOR WHOLE LINE 1 BELOW FEET?
    		{
    			goDown = false;									// THEN TOO BAD YOU CAN'T GO DOWN
    			i = pixels.length;
    		}
    	}
    	if(goDown) y+=5;
    ...
    public void momentum()
    {
		...
		{
	    	int[] pixels = new int[width];
	    	//check pixels right below feet (this is for ramps)
	    	owner.gameScreen.getRGBofSkeleton(x,y+height-1,width,1,pixels,0,width);
	    	for(int i = 0; i < pixels.length; i++)
	    	{
	    		if(pixels[i] == -16777216 || pixels[i] == -65281)
